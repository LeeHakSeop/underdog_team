...
--     OCR 결과의 게이트, 차량, 진행 중 작업오더를 자동 연결한다.
-- =====================================================================
...
DECLARE
    ...
    v_plate_number VARCHAR(30);
    v_exception_type VARCHAR(50);
    v_exception_message VARCHAR(255);
BEGIN
    v_plate_number := NULLIF(TRIM(NEW.recognized_plate), '');

    -- 1. OCR 번호판으로 차량을 찾는다. 공백 차이는 무시한다.
    IF v_plate_number IS NOT NULL THEN
        SELECT v.vehicle_id
        INTO v_vehicle_id
        FROM vehicle v
        WHERE REPLACE(v.plate_number, ' ', '') =
              REPLACE(v_plate_number, ' ', '')
        ORDER BY v.vehicle_id
        LIMIT 1;
    END IF;

    -- 2. 해당 트레일러의 가장 최근 진행 중 작업오더를 찾는다.
    IF v_vehicle_id IS NOT NULL THEN
        SELECT w.work_order_id
        INTO v_work_order_id
        FROM work_order w
        WHERE (
                w.trailer_vehicle_id = v_vehicle_id
                OR w.vehicle_id = v_vehicle_id
              )
          AND w.work_status IN (
                'DISPATCH_WAITING',
                'APPROVED',
                'GATE_IN',
                'IN_PROGRESS'
              )
        ORDER BY w.reserved_time DESC NULLS LAST,
                 w.work_order_id DESC
        LIMIT 1;
    END IF;

    -- 3. OCR 결과에 따라 예외 유형을 결정한다.
    IF COALESCE(NEW.is_success, FALSE) = FALSE THEN
        v_exception_type := 'PLATE_RECOGNITION_FAILED';
        v_exception_message := COALESCE(
            NULLIF(NEW.error_message, ''),
            '번호판을 정상적으로 인식하지 못했습니다.'
        );

    ELSIF NEW.confidence IS NULL OR NEW.confidence < 70 THEN
        v_exception_type := 'LOW_CONFIDENCE';
        v_exception_message :=
            '번호판 인식 신뢰도가 기준치보다 낮습니다. confidence='
            || COALESCE(NEW.confidence::TEXT, 'NULL');

    ELSIF v_vehicle_id IS NULL THEN
        v_exception_type := 'UNREGISTERED_VEHICLE';
        v_exception_message :=
            '등록되지 않은 번호판입니다: '
            || COALESCE(v_plate_number, '(인식값 없음)');

    ELSE
        RETURN NEW;
    END IF;

    -- 4. 동일 게이트·동일 예외가 최근 30초 안에 있으면 중복 저장하지 않는다.
    IF NOT EXISTS (
        SELECT 1
        FROM exception_log e
        WHERE e.gate_log_id IS NOT DISTINCT FROM NEW.gate_log_id
          AND e.exception_type = v_exception_type
...
-- 최초 OCR 결과 저장 시에만 실행한다.
-- 관리자 수동 수정 UPDATE로 예외가 중복 생성되는 것을 방지한다.
CREATE TRIGGER trg_plate_recognition_exception
AFTER INSERT ON plate_recognition
FOR EACH ROW

...
/* CODEX DRIVER DASHBOARD START */
const passStatus = computed(() => {
  if (!currentWorkOrder.value) {
    return '대기'
  }

  if (currentWorkOrder.value.isApproved && currentWorkOrder.value.canEnter) {
    return '출입 가능'
  }
...
const nextGuide = computed(() => {
  ...
  if (!currentWorkOrder.value.isApproved) {
    return '관리자 승인 후 게이트 출입이 가능합니다.'
  }

  if (!currentWorkOrder.value.canEnter) {
    return '기사 출입 가능 상태를 운송사 또는 관리자에게 확인하세요.'
  }

  return `${currentWorkOrder.value.sectorName || '지정 섹터'}로 이동 후 안내 메시지를 확인하세요.`
})
/* CODEX DRIVER DASHBOARD END */

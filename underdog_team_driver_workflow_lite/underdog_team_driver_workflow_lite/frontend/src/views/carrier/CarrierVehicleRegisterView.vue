...
const myCarrier = computed(() =>
  carriers.value.find(
    (carrier) => carrier.userId === currentUser?.userId,
  ),
)
...
const selectedDriver = computed(() =>
  assignableDrivers.value.find(
    (driver) =>
      driver.driverId === form.value.driverId,
  ),
)
...
const validate = () => {
  ...
  if (!selectedDriver.value) {
    throw new Error(
      '운송사 승인 완료 및 관리자 최종 승인 전 기사만 배정할 수 있습니다.',
    )
  }
  ...
  if (!form.value.chassisNo.trim()) {
    throw new Error('샤시 번호를 입력하세요.')
  }
}
...
const submitVehicle = async () => {
  ...
  try {
    ...
    await createVehicle({
      ...
      // 운송사 배정 차량은 트레일러로 강제 고정
      vehicleType: 'TRAILER',

      tonnage: form.value.tonnage,
      tractorNo: form.value.tractorNo.trim(),
      chassisNo: form.value.chassisNo.trim(),
    })

    message.value =
      '트레일러 배정 내용이 관리자에게 전달되었습니다. 최종 승인 후 출입 가능합니다.'
...
onMounted(() => {
  form.value.vehicleType = 'TRAILER'
  loadData()
})
</script>
        ...
        >
          ...
          <div class="field">
            ...
            <div class="inline-field">
              <input
                id="tractorNo"
                v-model.trim="form.tractorNo"
                placeholder="예: TR-999999"
              />
          ...
          <button
            class="primary-button full submit-button"
            type="submit"
            :disabled="
              saving ||
      ...
      <article class="panel">
        <div class="section-title">
          ...
          <span class="status-pill green">
            {{ assignableDrivers.length }}명
          </span>
        </div>

        <div
          v-if="myDrivers.length === 0"
          class="empty-box"
        >
          소속 기사가 없습니다.
        </div>

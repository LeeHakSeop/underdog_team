...
import { updateWorkOrderStatus } from '@/api/adminApi/workOrderApi'
...
const currentWorkOrder = computed(() => {
  return myWorkOrders.value.find((order) => order.workStatus !== 'COMPLETED') || myWorkOrders.value[0] || null
})

const changeStatus = async (order, status) => {
  await updateWorkOrderStatus(order.workOrderId, status)
        ...
        <table class="data-table">
          ...
          <tbody>
            ...
            >
              ...
              <td>
                <button v-if="order.isApproved && order.workStatus === 'APPROVED'" type="button" @click="changeStatus(order, 'IN_PROGRESS')">작업 시작</button>
                <button v-else-if="order.isApproved && order.workStatus === 'IN_PROGRESS'" class="primary-button" type="button" @click="changeStatus(order, 'COMPLETED')">작업 완료</button>
                <span v-else>{{ order.workStatus === 'COMPLETED' ? '완료됨' : '승인 대기' }}</span>
              </td>
            </tr>

...
const validateAccount = () => {
  ...
  if (
    ...
    accountForm.value.passwordConfirm
  ) {
    throw new Error(
      '비밀번호 확인이 일치하지 않습니다.',
    )
...
const validateDriver = () => {
  ...
  if (!driverForm.value.tonnage) {
    throw new Error('트랙터 톤수를 선택하세요.')
  }
  ...
  if (!driverForm.value.chassisNo.trim()) {
    throw new Error('샤시번호를 입력하세요.')
  }
}
...
const buildPayload = () => {
  const base = {
    ...
    displayName:
      signupRole.value === 'CARRIER'
        ? carrierForm.value.managerName.trim() ||
          carrierForm.value.carrierName.trim()
        : driverForm.value.driverName.trim(),
  }
  ...
  if (signupRole.value === 'CARRIER') {
    return {
      ...
      carrierContact:
        carrierForm.value.carrierContact.trim(),
  ...
  return {
    ...
    driverContact:
      driverForm.value.driverContact.trim(),

    carrierId:
      Number(driverForm.value.carrierId),

    // 기사 본인 트랙터
    plateNumber:
      driverForm.value.plateNumber.trim(),

    vehicleType: 'TRACTOR',

    tonnage:
      driverForm.value.tonnage,

    tractorNo:
      driverForm.value.tractorNo.trim(),

    chassisNo:
      driverForm.value.chassisNo.trim(),
  }
}
...
const submitSignup = async () => {
  ...
  try {
    ...
    submitMessage.value =
      signupRole.value === 'DRIVER'
        ? '회원가입 신청이 완료되었습니다. 운송사 승인과 관리자 최종 승인 후 로그인할 수 있습니다.'

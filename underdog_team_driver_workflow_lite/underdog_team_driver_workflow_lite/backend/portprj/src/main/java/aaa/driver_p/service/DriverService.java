...
import aaa.driver_p.model.CarrierDriverManagementDTO;
...
public class DriverService {
    ...
    public int delete(Long driverId) {
        ...
        if (driver == null) {
            throw new IllegalArgumentException("삭제할 기사 정보를 찾을 수 없습니다.");
        }
        if (workOrderMapper.countActiveByDriverId(driverId) > 0) {
            throw new IllegalStateException("진행 중이거나 승인 대기 중인 작업오더가 있어 삭제할 수 없습니다.");
        }
        workOrderMapper.clearDriverReference(driverId);
        driverMapper.unassignTrailers(driverId);
        driverMapper.deleteTractorByDriverId(driverId);
    ...
    public List<CarrierDriverManagementDTO> managementListByCarrierUserId(Long carrierUserId) {
        if (carrierUserId == null) {
            throw new RuntimeException("운송사 사용자 ID가 필요합니다.");
        }

        return driverMapper.findManagementListByCarrierUserId(carrierUserId);
    }
